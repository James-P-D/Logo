...
namespace LogicalParser
{
  public class Parser
  {
          ...
          else if (GetExistingObject(firstToken, objects) != null)
          {
            if (parameters >= 2)
            {
              ...
              if (param1.Equals(StringTokeniser.Assignment))
              {
                ...
                {
                  commands.Add(
                    ParseNumberAssignment(firstToken, CopyArray(stringToken.Tokens, 2), objects, stringToken));
                }
                else if (variable is BooleanVariable)
                {
                  commands.Add(ParseBooleanAssignment(firstToken, CopyArray(stringToken.Tokens, 2), objects,
                    stringToken));
                }
    ...
    private Command ParseNumberAssignment(string variableName, string[] subTokens, List<LogoObject> objects,
    ...
    private Command ParseBooleanAssignment(string variableName, string[] subTokens, List<LogoObject> objects,
      StringToken stringToken)
    {
      var variable = GetExistingObject(variableName, objects);
    ...
    private void PopOperatorAndEvaluate(Stack<OperatorType> operatorStack, Stack<Eval> evalStack,
      StringToken stringToken)
    {
      ...
      switch (currentOperator)
      {
        ...
        case OperatorType.LessThanOrEqual:
        {
          ...
          {
            ThrowError(
              $"'{currentOperator.ToString()}' can only be used with matching types (number and numbers or booleans and booleans)",
              stringToken);
          }
